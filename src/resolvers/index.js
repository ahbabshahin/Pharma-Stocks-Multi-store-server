...
const generateUpdateDescription = (oldData, newData, fields) => {
	...
	fields.forEach((field) => {
		if (oldData[field] !== newData[field] && newData[field] !== undefined) {
			changes.push(
				`${field} from "${oldData[field]}" to "${newData[field]}"`
			);
		}
	});
...
const paginate = async (
	...
	const body = await queryBuilder;
	return {
		body,
		total,
		hasMore: skip + body.length < total,
	};
};
...
const resolvers = {
	Query: {
		getUsers: async (_, {first, offset},{user}) => {
			...
			return await paginate(User, {}, { first, offset });
		},
		...
		business: async (_, { id }, { user }) => {
			...
			if (
				user.role === 'platform' ||
				(user.business && user.business._id.toString() === id)
			) {
		...
		searchBusinesses: async (
			...
			await createActivityLog(user, {
				name: 'Business',
				action: 'search',
				description: `Searched businesses with term "${searchTerm}"`,
			});
			return result;
		},
		products: async (_, { first, offset }, { user }) => {
			if (!user) throw new Error('Not authenticated');
			const query =
				user.role === 'platform' ? {} : { business: user.business._id };
			const result = await paginate(Product, query, { first, offset }, [
				'business',
			]);
			return {
				...result,
		...
		product: async (_, { id }, { user }) => {
			...
			if (
				user.role === 'platform' ||
				(user.business &&
					user.business._id.equals(product.business._id))
			) {
		...
		searchProducts: async (
			_,
			{ searchTerm, businessId, first, offset },
			{ user }
		) => {
			if (!user) throw new Error('Not authenticated');
			const query = {
		...
		customers: async (_, { first, offset }, { user }) => {
			...
			const query =
				user.role === 'platform' ? {} : { business: user.business._id };
			return await paginate(Customer, query, { first, offset }, [
				'business',
			]);
		},
		customer: async (_, { id }, { user }) => {
			...
			if (
				user.role === 'platform' ||
				(user.business &&
					user.business._id.equals(customer.business._id))
			) {
				return customer;
			}
			throw new Error('Unauthorized');
		...
		invoices: async (_, { first, offset }, { user }) => {
			...
			const query =
				user.role === 'platform' ? {} : { business: user.business._id };
			return await paginate(Invoice, query, { first, offset }, [
				'customer',
				'business',
				'items.product',
			]);
		...
		invoice: async (_, { id }, { user }) => {
			...
			if (
				user.role === 'platform' ||
				(user.business &&
					user.business._id.equals(invoice.business._id))
			) {
				return invoice;
			}
			throw new Error('Unauthorized');
		...
		sales: async (_, { first, offset }, { user }) => {
			...
			const query =
				user.role === 'platform' ? {} : { business: user.business._id };
			return await paginate(Sale, query, { first, offset }, [
				'customer',
				'business',
				'invoice',
			]);
		...
		sale: async (_, { id }, { user }) => {
			...
			if (
				user.role === 'platform' ||
				(user.business && user.business._id.equals(sale.business._id))
			) {
				return sale;
			}
			throw new Error('Unauthorized');
		...
		activityLogs: async (_, { first, offset }, { user }) => {
			...
			return await paginate(ActivityLog, query, { first, offset }, [
				'user',
			]);
		},
		me: async (_, __, { user }) => {
	...
	Mutation: {
		register: async (
			_,
			{ username, password, role, businessId },
			{ user }
		) => {
			...
			await createActivityLog(newUser, {
				name: 'User',
				action: 'create',
		...
		updateUser: async (
			_,
			{ id, username, password, role, businessId },
			{ user }
		) => {
			if (!user || user.role !== 'platform')
				throw new Error('Unauthorized');
			...
			await createActivityLog(user, {
				name: 'User',
				action: 'update',
				description: `User "${oldUser.username}" updated. ${description}`,
			});
			return updatedUser;
		},
		deleteUser: async (_, { id }, { user }) => {
			...
			await createActivityLog(user, {
				name: 'User',
				action: 'delete',
				description: `User "${deletedUser.username}" deleted`,
			});
			return true;
		},
		createBusiness: async (_, { name, address, phone, type }, { user }) => {
			...
			await createActivityLog(user, {
				name: 'Business',
				action: 'create',
				description: `Business "${name}" (BID: ${bid}) created`,
			});
			return business;
		},
		updateBusiness: async (
			...
			await createActivityLog(user, {
				name: 'Business',
				action: 'update',
				description: `Business "${oldBusiness.name}" (BID: ${oldBusiness.bid}) updated. ${description}`,
			});
		...
		deleteBusiness: async (_, { id }, { user }) => {
			...
			await createActivityLog(user, {
				name: 'Business',
				action: 'delete',
				description: `Business "${deletedBusiness.name}" (BID: ${deletedBusiness.bid}) deleted`,
			});
			return true;
		},
		createProduct: async (
			_,
			{ name, brand, sku, quantity, price, lowStockAmount },
			{ user }
		) => {
			if (!user || (!user.business && user.role !== 'platform'))
				throw new Error('Unauthorized');
			if (user.role === 'platform')
				throw new Error('Platform users must specify a business');
			const productData = {
				name,
				brand,
				sku,
			...
			productData.lowStockAlert =
				productData.quantity <= productData.lowStockAmount;
			const product = await Product.create(productData);
			await createActivityLog(user, {
				name: 'Product',
				action: 'create',
				description: `Product "${name}" (SKU: ${sku}, Brand: ${brand}) created`,
			});
			return product.populate('business');
		},
		updateProduct: async (
			_,
			{ id, name, brand, sku, quantity, price, lowStockAmount },
			{ user }
		) => {
			if (!user || (!user.business && user.role !== 'platform'))
				throw new Error('Unauthorized');
			...
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(oldProduct.business)
			) {
				throw new Error('Unauthorized');
			}
			...
			if (quantity !== undefined || lowStockAmount !== undefined) {
				...
				const newLowStockAmount =
					lowStockAmount !== undefined
						? lowStockAmount
						: oldProduct.lowStockAmount;
			...
			await createActivityLog(user, {
				name: 'Product',
				action: 'update',
				description: `Product "${oldProduct.name}" (SKU: ${oldProduct.sku}) updated. ${description}`,
			});
		...
		deleteProduct: async (_, { id }, { user }) => {
			...
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(deletedProduct.business)
			) {
				throw new Error('Unauthorized');
			}
		...
		createCustomer: async (
			...
			await createActivityLog(user, {
				name: 'Customer',
				action: 'create',
				description: `Customer "${name}" (Email: ${email}) created`,
			});
		...
		updateCustomer: async (
			...
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(oldCustomer.business)
			) {
				throw new Error('Unauthorized');
			}
			...
			await createActivityLog(user, {
				name: 'Customer',
				action: 'update',
				description: `Customer "${oldCustomer.name}" (Email: ${oldCustomer.email}) updated. ${description}`,
			});
		...
		deleteCustomer: async (_, { id }, { user }) => {
			...
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(deletedCustomer.business)
			) {
				throw new Error('Unauthorized');
			}
			...
			await createActivityLog(user, {
				name: 'Customer',
				action: 'delete',
				description: `Customer "${deletedCustomer.name}" (Email: ${deletedCustomer.email}) deleted`,
			});
			return true;
		},
		createInvoice: async (_, { customerId, items }, { user }) => {
			...
			await createActivityLog(user, {
				name: 'Invoice',
				action: 'create',
				description: `Invoice created for customer "${customer.name}" (Total: ${total})`,
			});
			await createActivityLog(user, {
				name: 'Sale',
				action: 'create',
				description: `Sale created for customer "${customer.name}" (Total: ${total})`,
			});
			return invoice
				.populate('customer')
				.populate('business')
		...
		updateInvoice: async (
			...
			if (!user || (!user.business && user.role !== 'platform'))
				throw new Error('Unauthorized');
			const oldInvoice = await Invoice.findById(id).populate(
				'items.product'
			);
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(oldInvoice.business)
			) {
				throw new Error('Unauthorized');
			}
			const updateData = {};
			...
			if (items) {
				...
				for (const item of items) {
					...
					if (
						!product ||
						!user.business._id.equals(product.business)
					) {
						throw new Error(
					...
					productQuantities.set(
						...
						(productQuantities.get(item.productId.toString()) ||
							0) + item.quantity
					);
				}
				...
				oldInvoice.items.forEach((item) => {
					oldQuantities.set(
						...
						(oldQuantities.get(item.product._id.toString()) || 0) +
							item.quantity
					);
				});
			...
			await createActivityLog(user, {
				name: 'Invoice',
				action: 'update',
				description: `Invoice updated. ${description}`,
			});
			if (updateData.total) {
				...
				await createActivityLog(user, {
					name: 'Sale',
					action: 'update',
					description: `Sale updated for invoice (Total: ${updateData.total})`,
				});
			}
		...
		deleteInvoice: async (_, { id }, { user }) => {
			if (!user || (!user.business && user.role !== 'platform'))
				throw new Error('Unauthorized');
			const deletedInvoice = await Invoice.findById(id).populate(
				'items.product'
			);
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(deletedInvoice.business)
			) {
				throw new Error('Unauthorized');
			}
			// Restock products
			...
			await createActivityLog(user, {
				name: 'Invoice',
				action: 'delete',
				description: `Invoice deleted (Total: ${deletedInvoice.total})`,
			});
			await createActivityLog(user, {
				name: 'Sale',
				action: 'delete',
				description: `Sale deleted for invoice (Total: ${deletedInvoice.total})`,
			});
			return true;
		},
		deleteSale: async (_, { id }, { user }) => {
			...
			if (
				user.role !== 'platform' &&
				!user.business._id.equals(deletedSale.business)
			) {
				throw new Error('Unauthorized');
			}
			...
			await createActivityLog(user, {
				name: 'Sale',
				action: 'delete',
				description: `Sale deleted (Total: ${deletedSale.total})`,
			});
			return true;
		},
	},
